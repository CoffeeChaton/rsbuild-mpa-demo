import { TrashIcon, ChevronDownIcon, ChevronRightIcon } from '@radix-ui/react-icons';
  ...
  return (
    <div className={`relative bg-white rounded-xl border border-slate-200 shadow-sm ${!group.isEnabled && 'opacity-50'}`}>
      ...
      <div className="p-4 flex items-center gap-3 border-b border-slate-50">
        ...
        <input
          type="checkbox" checked={group.isEnabled}
          onChange={() => onUpdate({ ...group, isEnabled: !group.isEnabled })}
        />
        <input
          value={group.listName} onChange={(e) => onUpdate({ ...group, listName: e.target.value })}
          className="font-bold text-sm w-32 bg-transparent outline-none" placeholder="組標題"
        />
        <input
          value={group.description} onChange={(e) => onUpdate({ ...group, description: e.target.value })}
          className="text-xs text-slate-400 flex-1 bg-transparent outline-none" placeholder="一行說明..."
        />
        <button onClick={onDelete} className="text-slate-300 hover:text-rose-500"><TrashIcon /></button>
      </div>
              ...
              <div key={idx} className="flex items-center gap-4">
                ...
                <div className="relative w-10 h-10 shrink-0">
                  <div className="absolute top-0 left-0 w-10 h-10 overflow-hidden z-10 hover:z-50 hover:w-30 hover:h-30 hover:shadow-xl transition-all">
                    {matKey ? <img src={`${import.meta.env.BASE_URL}/img/game/item/${matKey}.png`} className="w-full h-full object-contain" alt="" /> : <div className="w-full h-full" />}
                  </div>
                </div>

                <input
                  list="m-list" value={m.name}
                  onChange={(e) => {
                ...
                <button onClick={() => {
                  ...
                  onUpdate({ ...group, materials });
                }} className="text-slate-200 hover:text-rose-500"><TrashIcon /></button>
              </div>
            );
          })}
