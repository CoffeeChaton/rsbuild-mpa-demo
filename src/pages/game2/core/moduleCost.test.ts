...
describe("calculateModuleCost", () => {
	...
	it("sums all stages between moduleFrom and moduleTo for six-star", { timeout: 10000 }, () => {
	...
	it.each([
		...
		[4, 1, 2, 20_000],
		[4, 2, 3, 30_000],
	])("星級 %i 範圍 %i -> %i 消耗應為 %i", { timeout: 10000 }, (rarity, fromLevel, toLevel, expected) => {
		expect(calculateModuleCost(rarity, fromLevel, toLevel)).toBe(expected);
	});

	it("應正確處理跨級範圍", { timeout: 10000 }, () => {
	...
	it("returns zero for unsupported rarities or invalid ranges", { timeout: 10000 }, () => {
